...
| **Run instances** | branches × ⌈`extract.fetchesPerBranch` ÷ that source's batch size⌉ in Extract, plus the same division over `enrich.expertsFollowed × enrich.urlsPerExpert` in Enrichment, and fewer in practice since the dedupe drops the expert pages Extract already read |
