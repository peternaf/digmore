...
| **Runs** | `handle_vetting.mjs serve` once, for its own range. Then per handle, one script call — `hackernews.mjs vet` or `api.mjs twitter vet --posts <n>` — reads the recent activity it returned, judges topical relevance and on Twitter the voice, writes that handle's file, and validates it with `validate.mjs handle-vetting`. One handle finished before the next is started. **Reddit is the one exception, and only for the fetch**: `api.mjs reddit user` is called once with the whole range's names before any judging starts, because the endpoint takes a list — see `reddit.md`. The judging after it is the same one handle at a time. **On forums no source script and no network**: it reads the cached pages named in `documents` and judges from those alone |
...
| **Logs** | `cache/_progress/handle-vetter-<source>-<n>.log`, one per batch — `handle <n> of <n>: <handle>` · `vetting <handle> on <source>` · `checking <n> recent posts for the dead flag` (Hacker News) · `reading <n> cached tweets` (only when `needs_llm_judgment` is set) · `reading <n> cached pages for <handle>` (forums) · on Reddit, `fetching <n> profiles in one call` and `fetched <n> profiles: <n> ok, <n> failed` around the range fetch, so it is never mistaken for a hung agent |
