...
- **Pages the run could not read** — every URL whose Page Analyst came back `blocked`, one line each. A blocked page leaves nothing on disk, so its receipt is the only trace the run ever tried it; unrecorded, a walled subject and a genuinely undiscussed one are indistinguishable, and the report says "little was written about this" when the truth is "we could not get in".
- **Pages taken with WebFetch** — every URL whose receipt says `fetchedWith: WebFetch`. `fetch.mjs` was walled and WebFetch got through, but it shortens long pages silently and never says where it cut. Any claim drawn from one of these may be missing a tail nobody can see, and that belongs in the record rather than in nobody's head.
