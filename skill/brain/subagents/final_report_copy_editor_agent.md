...
2. **Then open the raw report and the finished CSVs.** Now you know what each flagged sentence was
   trying to say. Rewrite it — or remove the duplicate, keeping the earlier instance.

**Committing the list to disk before stage 2 is what makes the order real.** Without it, "read cold
first" is an instruction with nothing holding it in place: once the raw report is open you can no
longer tell what you did and did not understand unaided, and your own flags become unrecoverable. The
