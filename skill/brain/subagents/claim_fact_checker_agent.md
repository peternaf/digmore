...
| **Input data files** | none named in the dispatch. `serve` hands it each paragraph and the cached files its claims cite, and it opens those. Not `claim_index.json`, not the claim listing, not the rest of the summary |
