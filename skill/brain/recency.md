...
- **Reddit** — `api.mjs reddit search --time-window all --after-date <the cutoff preflight printed>`, passed by the caller on every search. **Both are hints the API forwards upstream, not filters it enforces**: a narrow window has been measured returning posts one and two years outside it, and no publish date comes back per result to catch it. Pass them anyway — they still skew the results recent — and do not describe what comes back as recent on the strength of having asked. Nothing re-checks a thread's own date; a run states its window rather than proving it.
