...
1. **Look at the subject.** Search the open web until you can say what these people call things and
   ...
   would use.
2. **Decompose it into angles.** 3–6 in a full run, exactly 2 in `--fast`. Each angle is one
...
- **The right count for the mode** — 3–6 full, exactly 2 in `--fast`. `ask` sets its own
  (`../modes.md`).
