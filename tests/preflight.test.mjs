...
test('a configuration reduced by --fast prints both values, and an unchanged one prints one', async () => {
  ...
  assert.match(rowFor('enrich.expertsFollowed'), /5 → 0/, 'zero is a skip and is shown as one');
  assert.match(rowFor('twitter.handlesDeepVetted'), /10 → 0/, 'zero is a skip and is shown as one');
